...
def min_max_scaling(age, bmi, physical_health, mental_health, sleep_time, bp, heart_rate):
	MAX_BMI             = 94.85
	MIN_BMI             = 12.02
	MAX_SLEEP_TIME      = 24.0
	MIN_SLEEP_TIME      = 1.0
	...
	scaled_heart_rate 	   = (int(heart_rate)-int(age))		
	scaled_bp			   = (int(bp)-MIN_BP)/(MAX_BP-MIN_BP)	

	return scaled_bmi, scaled_physical_health, scaled_mental_health, scaled_sleep_time, scaled_bp, scaled_heart_rate

def load_model():
	file = open("C:\heart\Flask\saved_model_lgbm.h5", "rb")
	model = pickle.load(file) 
	return model

def predict(sex, age, race, height, weight, smoking, alcohol, general_health, sleep_time, mental_health, physical_health, physical_activity, bp, heart_rate, serum_level, blood_sugar, stroke, diabetic, asthma, skin_cancer, kidney_disease):
	...
	bmi, physical_health, mental_health, sleep_time, bp, heart_rate, serum_level, blood_sugar = min_max_scaling(age, bmi, physical_health, mental_health, sleep_time, bp, heart_rate, serum_level, blood_sugar)

	df_dict = {"BMI": bmi, "Smoking":smoking, "AlcoholDrinking":alcohol, "Stroke": stroke, "PhysicalHealth": physical_health, "MentalHealth":mental_health, "Sex":sex, "AgeCategory":age, "Race":race, "Diabetic":diabetic, "PhysicalActiviy":physical_activity, "GenHealth":general_health, "SleepTime":sleep_time, "Asthma":asthma, "KidneyDisease":kidney_disease, "SkinCancer":skin_cancer}
